...
def createMenu():
    ...
    menu_size = 150  # Tamanho do menu
    menu_x, menu_y = 20, 480 - menu_size - 20  # Coordenadas do canto inferior esquerdo

    curlTlx, curlTly = menu_x, menu_y
    curlBrx, curlBry = menu_x + menu_size, menu_y + menu_size
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            text_width, text_height = cv2.getTextSize('BICEP CURL', cv2.FONT_HERSHEY_SIMPLEX, 1, 1)[0]
            text_x = int((menu_size - text_width) / 2) + menu_x
            text_y = int((menu_size + text_height) / 2) + menu_y
            cv2.putText(image, 'BICEP CURL', (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1, cv2.LINE_AA)
